...
async function main() {
  ...
  console.log(`BLOCKCHAIN_NETWORK_ID=${(await ethers.provider.getNetwork()).chainId}`);
  console.log(`BLOCKCHAIN_PROVIDER_URL=${ethers.provider._getConnection().url}`);
  ...
  const frontendConfigFile = path.join(deploymentsDir, `${networkName}-frontend-config.json`);
