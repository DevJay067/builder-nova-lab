...
class SupabaseService {
  ...
  static async initializeDatabase(): Promise<void> {
    ...
    try {
      ...
      const { data, error } = await client.from('health_records').select('id').limit(1);
      
      if (error && error.code === 'PGRST116') {
        console.log('⚠️ Tables not yet created in Supabase. Please run the database migration.');
      } else {
  ...
  static async initializeStorage(): Promise<void> {
    ...
    try {
      ...
      const { data: bucketData, error: bucketError } = await client.storage
        .createBucket('health-vault', {
          public: false, // Private bucket for security
          allowedMimeTypes: ['application/json', 'text/plain', 'application/pdf', 'image/*'],
      ...
      const { data: recordsBucket, error: recordsError } = await client.storage
        .createBucket('medical-records', {
          public: false,
      ...
      if (recordsError && !recordsError.message.includes('already exists')) {
        console.error('❌ Error creating medical-records bucket:', recordsError);
      } else {
   ...
   */
  static async storeHealthRecord(record: Omit<HealthRecord, 'id' | 'created_at' | 'updated_at'>): Promise<{ success: boolean; recordId?: string; error?: string }> {
    const client = this.initialize();
   ...
   */
  static async storeInVault(patientId: string, data: any, filename?: string): Promise<{ success: boolean; path?: string; error?: string }> {
    const client = this.initialize();
   ...
   */
  static async retrieveFromVault(path: string): Promise<{ success: boolean; data?: any; error?: string }> {
    const client = this.initialize();
   ...
   */
  static async getHealthRecords(patientId: string): Promise<{ success: boolean; records?: HealthRecord[]; error?: string }> {
    const client = this.initialize();
    ...
    try {
      ...
      console.log(`✅ Retrieved ${data?.length || 0} health records for patient: ${patientId}`);
   ...
   */
  static async storeSecureRecord(record: Omit<SecureDataRecord, 'id' | 'created_at' | 'updated_at'>): Promise<{ success: boolean; recordId?: string; error?: string }> {
    const client = this.initialize();
    ...
    try {
      ...
      const vaultResult = await this.storeInVault(
        record.patient_id,
        { encryptedData: record.encrypted_data, metadata: { keyId: record.key_id, checksum: record.checksum } },
   ...
   */
  static async authenticateUser(email: string, password: string): Promise<{ success: boolean; user?: any; session?: any; error?: string }> {
    const client = this.initialize();
   ...
   */
  static async registerUser(email: string, password: string, metadata?: any): Promise<{ success: boolean; user?: any; error?: string }> {
    const client = this.initialize();
  ...
  static async getStatistics(): Promise<any> {
    ...
    try {
      ...
      const [healthRecords, secureRecords, vaultFiles] = await Promise.all([
        client.from('health_records').select('id', { count: 'exact', head: true }),
        client.from('secure_data_records').select('id', { count: 'exact', head: true }),
  ...
  static async testConnection(): Promise<boolean> {
    ...
    try {
      ...
      const { error } = await client.from('health_records').select('id').limit(1);
      return !error;
    } catch (error) {
   ...
   */
  static async getStorageUsage(): Promise<{ vaultUsage: number; recordsUsage: number; totalFiles: number }> {
    const client = this.initialize();
    ...
    try {
      ...
      const vaultUsage = vaultFiles.data?.reduce((total, file) => total + (file.metadata?.size || 0), 0) || 0;
      const recordsUsage = recordFiles.data?.reduce((total, file) => total + (file.metadata?.size || 0), 0) || 0;
      const totalFiles = (vaultFiles.data?.length || 0) + (recordFiles.data?.length || 0);
