...
export class SplitKeyAuthService {
  ...
  static async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!supabaseUrl || !supabaseKey) {
        console.warn(
          "⚠️ Supabase credentials not found, using fallback storage",
        );
      ...
      console.log(
        "✅ Split-key authentication service initialized with Supabase",
      );
  ...
  private static async createTables(): Promise<void> {
    ...
    try {
      ...
      if (error) {
        console.warn(
          "⚠️ Could not create tables via RPC, tables may already exist",
        );
      }
    } catch (error) {
  ...
  static async registerUser(
    ...
    try {
      ...
      console.log(
        `✅ User ${username} registered with split-key authentication`,
      );
   ...
   */
  static verifySessionToken(sessionToken: string): {
    valid: boolean;
    userId?: string;
  } {
    try {
  ...
  private static async storeUser(user: SplitKeyUser): Promise<boolean> {
    if (this.supabase) {
      try {
        const { error } = await this.supabase.from("split_key_users").insert([
          {
            id: user.id,
            email: user.email,
