...
class PersonalizedMedicalContextService {
  ...
  static extractMedicalConditions(healthRecords: any[]): MedicalCondition[] {
    ...
    healthRecords.forEach((record) => {
      if (record.metadata) {
        ...
        const symptoms = this.extractSymptomsFromText(
   ...
   */
  private static inferConditionFromMedication(
    medication: string,
  ): string | null {
   ...
   * Deduplicate and merge similar conditions
   */
  private static deduplicateConditions(
      ...
      if (existing) {
        ...
        if (
          ...
          (!existing.lastUpdated ||
            condition.lastUpdated > existing.lastUpdated)
        ) {
   ...
   */
  static async createPersonalizedContext(
    sessionToken: string,
  ...
  private static extractCurrentMedications(records: any[]): string[] {
    ...
    records.forEach((record) => {
      if (record.metadata?.medications) {
        record.metadata.medications.forEach((med: string) =>
          medications.add(med),
        );
      }
    });
  ...
  private static extractAllergies(records: any[]): string[] {
    ...
    records.forEach((record) => {
      if (record.metadata?.allergies) {
        record.metadata.allergies.forEach((allergy: string) =>
  ...
  private static createContextualPrompt(
    ...
    prompt +=
      "IMPORTANT: When providing health advice, always consider these existing conditions and medications. Provide specific guidance relevant to this patient's medical history.";
   ...
   */
  private static createSearchEnhancers(
    conditions: MedicalCondition[],
  ): string[] {
    ...
    personalizedContext.searchEnhancers.forEach((enhancer) => {
      if (queryLower.includes(enhancer.toLowerCase())) {
        const relatedConditions = personalizedContext.medicalConditions
          .filter((condition) =>
            condition.name.toLowerCase().includes(enhancer.toLowerCase()),
          )
          .map((condition) => condition.name);
   ...
   */
  private static isQueryRelatedToCondition(
    query: string,
    condition: string,
  ): boolean {
  ...
  private static createPersonalizedPrompt(
    ...
    prompt +=
      "Please provide health advice that specifically considers this patient's medical history and current conditions. ";
    prompt +=
      "Include relevant precautions, medication interactions, and condition-specific recommendations.";
