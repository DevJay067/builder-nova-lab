...
router.post("/register", authLimiter, async (req, res) => {
  try {
    const { email, username, firstName, lastName, dateOfBirth, phone } =
      req.body;
    ...
    const result = await SplitKeyAuthService.registerUser(email, username, {
      firstName,
      lastName,
      dateOfBirth,
      phone,
...
router.post("/login", authLimiter, async (req, res) => {
  try {
    ...
    // Authenticate user
    const result = await SplitKeyAuthService.authenticateUser(
      userId,
      clientKeyHalf,
    );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const file = req.file;
      console.log(
        `📤 Uploading medical file: ${file.originalname} (${file.size} bytes)`,
      );
      ...
      if (!metadataResult.success) {
        console.warn(
          "⚠️ Failed to store metadata in Supabase, but file uploaded to IPFS",
        );
      }

      console.log(
        `✅ Medical file uploaded successfully. CID: ${ipfsResult.cid}`,
      );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `✅ Medical record downloaded and decrypted: ${record.originalName}`,
      );
...
router.post("/verify-key", authLimiter, async (req, res) => {
  try {
    ...
    const result = await SplitKeyAuthService.validateAndGetFullKey(
      userId,
      clientKeyHalf,
    );
