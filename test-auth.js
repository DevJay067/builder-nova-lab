...
async function testAuth() {
  ...
  try {
    ...
    if (registerResult.success) {
      ...
      console.log(
        "   Session Token:",
        registerResult.user?.sessionToken ? "[SET]" : "[NOT SET]",
      );
      console.log(
        "   Secure System:",
        registerResult.user?.secureSystemActivated ? "ACTIVE" : "INACTIVE",
      );
      ...
      if (loginResult.success) {
        ...
        console.log(
          ...
          storeResult.success ? "PASS" : "FAIL",
        );

        // Test 6: Retrieve health records
        const retrieveResponse = await fetch(
          `${baseUrl}/api/auth/data-access/records`,
          {
            headers: { "x-session-token": sessionToken },
          },
        );
        ...
        console.log(
          ...
          retrieveResult.success ? "PASS" : "FAIL",
        );
  ...
  try {
    ...
    console.log(
      "   Database Size:",
      statsData.stats?.databaseSize || 0,
      "bytes",
    );
  } catch (error) {
