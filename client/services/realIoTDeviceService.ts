...
class RealIoTDeviceService {
  ...
  private checkDeviceSupport() {
    ...
    this.isSupported =
      hasBluetoothAPI && hasServiceWorker && (isSecureContext || isHTTPS);
  ...
  async scanAndConnectBluetooth(): Promise<DeviceConnection[]> {
    ...
    if (!navigator.bluetooth) {
      throw new Error(
        "Web Bluetooth API not available. Please use Chrome or Edge browser.",
      );
    }
    ...
    try {
      ...
      if (error.name === "SecurityError") {
        throw new Error(
          "Bluetooth access blocked. Please enable Bluetooth permissions in your browser settings or use HTTPS.",
        );
      } else if (error.name === "NotFoundError") {
        throw new Error(
          "No compatible devices found. Make sure your device is in pairing mode and nearby.",
        );
        ...
        throw new Error(
          "Bluetooth blocked by browser policy. Try opening this page directly in a new tab.",
        );
      }

      throw new Error(
        `Bluetooth connection failed: ${error.message || "Unknown error"}`,
      );
    }
  ...
  private async setupHealthServices(
    ...
    try {
      const bpService = await server.getPrimaryService(
        HEALTH_SERVICE_UUIDS.BLOOD_PRESSURE,
      );
      const bpCharacteristic = await bpService.getCharacteristic(
        CHARACTERISTIC_UUIDS.BLOOD_PRESSURE_MEASUREMENT,
      );

      await bpCharacteristic.startNotifications();
      bpCharacteristic.addEventListener(
        "characteristicvaluechanged",
        (event) => {
          const value = (event.target as BluetoothRemoteGATTCharacteristic)
            .value;
      ...
      console.log(
        `🩺 Blood pressure monitoring enabled for ${connection.name}`,
      );
    } catch (e) {
  ...
  private initializeWebSocketConnection() {
    ...
    if (isDevelopment) {
      console.log(
        "⚠️ WebSocket skipped in development mode. Using local simulation instead.",
      );
      return;
    }
    ...
    try {
      ...
      this.websocket.onerror = (error: Event) => {
        ...
        const errorDetails = {
          ...
          readyStateText: this.getWebSocketStateText(
        ...
        if (error instanceof ErrorEvent) {
          ...
          if (target.readyState !== undefined)
            errorDetails.readyState = target.readyState;
        }
    ...
    if ("Accelerometer" in window) {
      try {
        ...
        accel.addEventListener("reading", () => {
          ...
          const steps = this.calculateStepsFromAcceleration(
            accel.x,
            accel.y,
            accel.z,
          );
  ...
  private parseBloodPressureData(value: DataView): {
    systolic: number;
    diastolic: number;
  } {
  ...
  private calculateStepsFromAcceleration(
    x: number,
    y: number,
    z: number,
  ): number {
  ...
  private detectDeviceType(deviceName: string): DeviceConnection["type"] {
    ...
    if (name.includes("blood pressure") || name.includes("omron"))
      return "blood_pressure";
    if (name.includes("pulse") || name.includes("oximeter"))
  ...
  async connectBoAtDevice(): Promise<DeviceConnection | null> {
    try {
      ...
      if (error.name === "SecurityError") {
        throw new Error(
          "Bluetooth access denied. Please open this page in a new tab or enable Bluetooth permissions.",
        );
      } else if (error.name === "NotFoundError") {
        throw new Error(
          "No boAt devices found. Make sure your boAt watch is in pairing mode and nearby.",
        );
      } else if (error.name === "NotAllowedError") {
        throw new Error(
          "Bluetooth permission denied. Please allow access when prompted.",
        );
      }
  ...
  async connectFitbitDevice(): Promise<DeviceConnection | null> {
    try {
      ...
      if (error.name === "SecurityError") {
        ...
        throw new Error(
          "No Fitbit devices found. Make sure your Fitbit is in pairing mode and nearby.",
        );
      } else if (error.name === "NotAllowedError") {
        throw new Error(
          "Bluetooth permission denied. Please allow access when prompted.",
  ...
  connectToHealthStream(wsUrl?: string): void {
    ...
    try {
      const url =
        wsUrl ||
        `${window.location.protocol === "https:" ? "wss:" : "ws:"}//${window.location.host}/health-stream`;
