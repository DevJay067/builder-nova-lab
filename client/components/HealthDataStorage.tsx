...
export class HealthDataStorage {
   ...
   */
  static async storeHealthData(data: HealthDataRecord): Promise<StorageResponse> {
    try {
      ...
      if (result.success) {
        console.log(`✅ Health data stored in cloud vault: ${result.vaultPath}`);
        return result;
      } else {
  ...
  static async getHealthData(sessionToken?: string): Promise<{
    ...
    try {
      console.log('🔍 Retrieving health data from Supabase cloud storage vault...');
      ...
      if (result.success) {
        console.log(`✅ Retrieved ${result.totalRecords || 0} records from cloud vault`);
        return result;
      } else {
   ...
   */
  static async storeVitalSigns(data: {
    ...
    glucoseLevel?: number;
    [key: string]: any;
  }, sessionToken?: string): Promise<StorageResponse> {
   ...
   */
  static async storeMedication(data: {
    name: string;
    dosage: string;
    frequency: string;
    startDate: string;
   ...
   */
  static async storeAppointment(data: {
    date: string;
    time: string;
    doctor: string;
    specialty?: string;
   ...
   * Store lab results
   */
  static async storeLabResults(data: {
    testName: string;
    results: any;
    date: string;
   ...
   */
  static async storeGeneralRecord(data: {
    title: string;
    description?: string;
    category: string;
    data: any;
