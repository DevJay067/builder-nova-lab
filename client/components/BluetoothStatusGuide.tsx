...
export default function BluetoothStatusGuide({ className = '' }: BluetoothStatusGuideProps) {
  ...
  const getStatusColor = () => {
    if (bluetoothStatus.checking) return 'bg-blue-50 text-blue-700 border-blue-200';
    if (!bluetoothStatus.supported || !bluetoothStatus.available || bluetoothStatus.permission === 'denied') {
      return 'bg-red-50 text-red-700 border-red-200';
    }
    if (bluetoothStatus.permission === 'granted') return 'bg-green-50 text-green-700 border-green-200';
  ...
  return (
    <Card className={`${className} border-0 bg-white/80 backdrop-blur-sm rounded-2xl`}>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3">
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={bluetoothStatus.supported ? 'text-green-600' : 'text-red-600'}>
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            <span className="font-medium text-gray-700">Bluetooth Hardware</span>
            <div className="flex items-center gap-2">
              ...
              <span className={bluetoothStatus.available ? 'text-green-600' : 'text-red-600'}>
          ...
          <div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className={
                bluetoothStatus.permission === 'granted' ? 'text-green-600' : 
                bluetoothStatus.permission === 'denied' ? 'text-red-600' : 'text-yellow-600'
              }>
                {bluetoothStatus.permission === 'granted' ? 'Granted' :
                 bluetoothStatus.permission === 'denied' ? 'Denied' :
                 bluetoothStatus.permission === 'prompt' ? 'Will Ask' : 'Unknown'}
              </span>
            </div>
          </div>
        </div>
              ...
              <div className="space-y-3">
                ...
                {!bluetoothStatus.supported && (
                  <div className="text-sm text-blue-700">
                    Please use Chrome or Edge browser for the best Bluetooth support, or try Demo Mode to test the interface.
                  </div>
                )}
              </div>
            </AlertDescription>
        ...
        <div className="flex gap-2">
          <Button 
            ...
            className="flex-1"
          >
            <RefreshCw className={`w-4 h-4 mr-2 ${bluetoothStatus.checking ? 'animate-spin' : ''}`} />
            Check Again
          </Button>
        </div>
      </CardContent>
