...
export default function BluetoothHealthMonitor() {
  ...
  const connectToDevice = async (bluetoothDevice: BluetoothDevice) => {
    ...
    const existingDevice = devices.find(d => d.bluetoothId === bluetoothDevice.id);
    if (!existingDevice) {
  ...
  const startMonitoring = async (device: BluetoothDevice, server: BluetoothRemoteGATTServer) => {
    try {
  ...
  const monitorHeartRate = async (server: BluetoothRemoteGATTServer, deviceId: string) => {
    try {
  ...
  const getDeviceType = (name: string): HealthDevice['type'] => {
    ...
    if (lowerName.includes('fitbit') || lowerName.includes('tracker')) return 'fitness_tracker';
  ...
  const getDeviceIcon = (type: HealthDevice['type']) => {
    switch (type) {
      ...
      case 'pulse_oximeter': return Droplets;
        ...
        <CardContent className="space-y-4">
          {!bluetoothSupported ? (
            <Alert className="border-red-200 bg-red-50">
              ...
              <AlertDescription>
                Bluetooth is not supported in this browser. Please use Chrome, Edge, or a supported browser.
              </AlertDescription>
            </Alert>
          ) : !bluetoothEnabled ? (
            ...
            <Card className="border-gray-200">
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {['Heart Rate', 'Blood Pressure', 'Temperature', 'Oxygen Saturation', 'Steps', 'Sleep'].map((permission) => (
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center space-x-2">
            ...
            <span>Connected Devices ({devices.filter(d => d.connected).length})</span>
          </CardTitle>
        </CardHeader>
        <CardContent>
          {devices.length === 0 ? (
                  ...
                  <div key={device.id} className="flex items-center justify-between p-4 rounded-lg border bg-card">
                    <div className="flex items-center space-x-4">
                      <div className={`p-2 rounded-lg ${
                        device.connected ? 'bg-green-100 text-green-600' :
                        device.connecting ? 'bg-yellow-100 text-yellow-600' :
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <Badge variant={device.connected ? "default" : device.connecting ? "secondary" : "outline"}>
                        {device.connecting ? 'Connecting...' : device.connected ? 'Connected' : 'Disconnected'}
                      </Badge>
                      {device.connected && (
                        <Button
                          variant="outline"
