      ...
      if (process.env.NODE_ENV !== "production") {
        ...
        server.middlewares.use("/api/auth/demo-login", (req, res, next) => {
          if (req.method === "POST") {
            ...
            res.setHeader(
              "Set-Cookie",
              `healthchain_session=${demoSessionToken}; Path=/; Max-Age=3600; SameSite=Strict`,
            );
            res.statusCode = 200;
            res.end(
              JSON.stringify({
        ...
        server.middlewares.use("/api/auth/verify", (req, res, next) => {
          if (req.method === "GET") {
            ...
            const sessionToken =
              authHeader?.replace("Bearer ", "") || sessionHeader;
              ...
              JSON.stringify({
                ...
                records: [
                  ...
                  },
                  {
                    ...
                    description:
                      "Comprehensive metabolic panel including cholesterol",
                    date: "2024-02-10",
                    doctor: "Dr. Johnson",
            ...
            try {
              ...
              fallbackApp.post("/api/auth/demo-login", (req, res) => {
                ...
                const demoUser = {
                  ...
                  username: "demo_user",
                  userHash:
                    "demo_hash_" + Math.random().toString(36).substr(2, 9),
                  sessionToken: demoSessionToken,
                  firstName: "Demo",
                  lastName: "User",
                  email: "demo@example.com",
              ...
              console.error(
                "❌ Failed to create fallback server:",
                fallbackError,
              );
            }
          });
